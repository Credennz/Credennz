import React from 'react'

export default function testimonial() {
  return (
    <div>
        <div class="h-testimonials-container">
            <div class="testimonials-header">
                <h6>TESTIMONIALS</h6>
                <h1>Hear What Our Clients<br/> Have to Say</h1>
                <div class="h-testimonials-subheader"></div>
            </div>
    
            <div class="h-testimonials-grid1">
                {/* <!-- Testimonial 1 --> */}
                <div class="h-testimonial-card h-testimonial-card1">
                    <div class="h-testimonial-content">
                        "Incredible attention to detail. They meticulously review every aspect of our digital strategy."
                    </div>
                    <hr/>
                    <div class="h-testimonial-author">
                        <div class="author-image">
                            <img src="../img/img1.png" alt="Alex Rodriguez"/>
                        </div>
                        <div class="author-info">
                            <p class="author-name">Alex Rodriguez</p>
                            <p class="author-title">Director of Operations, Global</p>
                        </div>
                    </div>
                </div>
    
                {/* <!-- Testimonial 2 --> */}
                <div class="h-testimonial-card h-testimonial-card2">
                    <div class="h-testimonial-content">
                        "They transformed our business! Their creative solutions and strategic planning led to consistent
                        growth."
                    </div>
                    <hr/>
                    <div class="h-testimonial-author">
                        <div class="author-image">
                            <img src="../img/img2.png" alt="Jake Thompson"/>
                        </div>
                        <div class="author-info">
                            <p class="author-name">Jake Thompson</p>
                            <p class="author-title">Founder & CEO, Digital Trends</p>
                        </div>
                    </div>
                </div>
    
                {/* <!-- Testimonial 3 --> */}
                <div class="h-testimonial-card h-testimonial-card3">
                    <div class="h-testimonial-content">
                        "They transformed our social media game—our sales have never been better!"
                    </div>
                    <hr/>
                    <div class="h-testimonial-author">
                        <div class="author-image">
                            <img src="../img/img3.png" alt="Emily Chen"/>
                        </div>
                        <div class="author-info">
                            <p class="author-name">Emily Chen</p>
                            <p class="author-title">E-commerce Entrepreneur, Lead</p>
                        </div>
                    </div>
                </div>
            </div>
    
            <div class="h-testimonials-grid2">
                {/* <!-- Testimonial 4 --> */}
                <div class="h-testimonial-card h-testimonial-card4">
                    <div class="h-testimonial-content">
                        "Our online presence has skyrocketed since we started working with them."
                    </div>
                    <hr/>
                    <div class="h-testimonial-author">
                        <div class="author-image">
                            <img src="../img/img4.png" alt="Samantha Williams"/>
                        </div>
                        <div class="author-info">
                            <p class="author-name">Samantha Williams</p>
                            <p class="author-title">Marketing Manager, Tech</p>
                        </div>
                    </div>
                </div>
    
                {/* <!-- Testimonial 5 --> */}
                <div class="h-testimonial-card h-testimonial-card5">
                    <div class="h-testimonial-content">
                        "Their tailored approach made our small business stand out in a big way!"
                    </div>
                    <hr/>
                    <div class="h-testimonial-author">
                        <div class="author-image">
                            <img src="../img/img5.png" alt="Sophie Patel"/>
                        </div>
                        <div class="author-info">
                            <p class="author-name">Sophie Patel</p>
                            <p class="author-title">Founder
                                , Urban Food Feed</p>
                        </div>
                    </div>
                </div>
    
                {/* <!-- Testimonial 6 --> */}
                <div class="h-testimonial-card h-testimonial-card6">
                    <div class="h-testimonial-content">
                        "Exceptional analytics! Their insights revolutionized our marketing."
                    </div>
                    <hr/>
                    <div class="h-testimonial-author">
                        <div class="author-image">
                            <img src="../img/img6.png" alt="Chris Reynolds"/>
                        </div>
                        <div class="author-info">
                            <p class="author-name">Chris Reynolds</p>
                            <p class="author-title">VP of Sales, Data Driven</p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
  )
}
